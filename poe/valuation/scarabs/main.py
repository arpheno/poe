...
def scarab_orb_of_horizon(prices):
    ...
    analysis = pd.concat(
        [profit_analysis(df, key, orb_of_horizon_price=orb_of_horizon_price) for key in df.keys()]
    ).reset_index()
...
def profit_analysis(df, key, orb_of_horizon_price):
    analysis = pd.concat(
        [profitability(df, key, orb_of_horizon_price), profit(df, key, orb_of_horizon_price)],
        axis=1,
