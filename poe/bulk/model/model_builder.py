...
def model_builder(opt):
    ...
    fe = feature_extraction_factory(
        opt.FeatureExtraction, opt.input_channel, opt.output_channel
    )
    model = Model(opt, fe, predictor=predictor)
    model = torch.nn.DataParallel(model).to(device)
    model.load_state_dict(
        torch.load(
            f"{Path(__file__).resolve().parent}/{opt.saved_model}", map_location=device
        )
    )
    model.eval()
    return model
