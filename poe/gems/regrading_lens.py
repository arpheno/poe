...
from poe.gems.adps import adps
...
def regrading_lens(relevant_gems, regrading_lens_cost):
    ...
    df["quality"] = (
        df.name.str.startswith("Anomalous") | df.name.str.startswith("Divergent") | df.name.str.startswith("Phantasmal")
    )
    df.loc[df.quality == False, "quality"] = "Superior"
    df.loc[df.quality == True, "quality"] = df[df.quality == True].name.str.split().str[0]
    df["basegem"] = (
        df.name.str.replace("Anomalous", "").str.replace("Divergent", "").str.replace("Phantasmal", "").str.strip()
    )
    df = df.set_index(["quality", "basegem"])
    gems = df
    df = pd.read_csv(f"{Path(__file__).resolve().parent}/gem_quality.csv")
    qual_weight_map = df.drop_duplicates(keep="first").set_index(["Type", "Item"]).fillna(0)["Weight"]
    qual_weight_map.head()
    gems["weight"] = qual_weight_map
    inter = (
        gems.reset_index()
        .groupby(["basegem"])
        .apply(lambda x: (x.set_index("quality")["chaosValue"].to_dict(), x.set_index("quality")["weight"].to_dict()))
    )
    result = pd.concat([r for r in inter.apply(lambda x: adps(x[1], x[0], regrading_lens_cost))], keys=inter.index)
    ...
    result["to"] = result.apply(
        lambda x: "Anomalous" if x.value == x.Anomalous else "Divergent" if x.value == x.Divergent else "Phantasmal",
        axis=1,
    )
    result["actionable"] = result["from"] != result["to"]
    endresult = result.sort_values(["actionable", "value"], ascending=False).query("actionable == True and value>0")
