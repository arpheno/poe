...
class Layouter:
    ...
    def layout(self, register: pd.DataFrame) -> pd.DataFrame:
        register["currency"] = register.final_price_chaos.map(lambda x: "exalted" if x > self.exalt_value else "chaos")
        funcs = {"chaos": self.turn_to_fraction, "exalted": self.turn_to_exalt}
        register["final_price"] = register.apply(
            lambda item: funcs[item.currency](item.final_price_chaos, item.stack_size), axis=1
        )
        return register
    ...
    def turn_to_exalt(self, final_price_chaos, stack_size):
        ...
        numerator, denominator = (
            (1, round(ratio)) if ((ratio := stack_size / numerator) > 1) else (numerator, stack_size)
        )
        return FakeFraction(round(numerator, 2), denominator)

    def turn_to_fraction(self, final_price_chaos, stack_size):
        best_fraction = Fraction(final_price_chaos / stack_size).limit_denominator(stack_size)
