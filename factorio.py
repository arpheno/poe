...
interburbul(10,[0.1,0.6,0.1],[0.8,-0.1,0.3],[0.3,0.9,.8],[5,2])
interburbul(50,[1.618033,0.9887498,0.48204586],[2.414213,0.562373,0.950488],[3.302775,0.6377319,0.9464655],[43,32])
...
def translate(f:str):
    ...
    inputs = np.array([1 if y in [arcospheres.index(x.upper()) for x in inputs.split()] else 0 for y in range(8)])
    outputs = np.array([1 if y in [arcospheres.index(x.upper()) for x in outputs.split()] else 0 for y in range(8)])
